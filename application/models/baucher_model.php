...
class Baucher_model extends CI_Model {
    ...
    public function get_by_taller($id) {
        $result = $this->db->query('SELECT u.nombre, u.paterno, u.materno,u.email, tu.usuario , b.folio , b.status FROM usuarios AS u INNER JOIN baucher AS b ON b.usuario_id = u.id inner join  tipo_usuario as tu on tu.id=u.tipo_usuario_id WHERE b.taller_semestre_id = ' . $id . ' AND (b.`status` = 0 OR b.`status` = 1) GROUP BY b.id ORDER BY u.paterno ASC');
        return ($result->num_rows() > 0) ? $result->result_array() : false;
    }
