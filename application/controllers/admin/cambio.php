...
class Cambio extends CI_Controller {
    ...
    public function index($baucher_id = null , $taller_id = null) {
        if($baucher_id !== null && $taller_id !== null){
            ...
            $baucher_taller = $this->baucher_model->get_by_baucher_taller($baucher_id,$taller_id);
            if($baucher_taller){
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if (is_array($taller)) {
                if ($status == false || $status['status'] == 3) {
                    $count = $this->baucher_model->count_insc($taller_new);
                    if ($count >= $taller['cupo']) {
                        $errors[] = 'El cupo est&aacute; lleno para ' . $taller['taller'];
                        //$errors[count($errors) - 1]['id'] = $id;
                        $exito = false;
                    }
                    if ($user_type == 4 && $this->baucher_model->count_trabajadores_insc($taller_new) >= 2) {
            ...
            if($exito){
                if ($this->baucher_model->update_by_baucher_taller($baucher_id,$taller_ant,$taller_new)) {
