    ...
    public function index($token) {
        ...
        try{
            ...
            if(isset($asistencia->data->id)){
                ...
                if($data){
                    $evento = $this->eventos_model->get($data['evento_id']);
                    if(time() > strtotime($evento['fecha']) - 10800){
                        ...
                        echo json_encode(array("status" => "ERROR" , 'code' => 0 , 'alumno' => $data,'message' =>'Aún no se puede registrar la asistencia'));
                    }
